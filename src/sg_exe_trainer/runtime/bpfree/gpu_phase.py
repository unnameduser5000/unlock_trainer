...
from sg_exe_trainer.runtime.bpfree.gpu_stage import BPFreePipelineStageV0
...
def run_phase_schedule_v3(
    *,
    ...
    stage = BPFreePipelineStageV0(
        ...
        max_pending_sends=2,
        recv_inflight_depth=recv_inflight_depth,
    ...
    schedule = ScheduleBPFreeBodySendHeadV1(
        ...
        perf_minimal_metrics=perf_minimal_metrics,
        window_input_staging=window_input_staging,
    )
    ...
    try:
        with metrics_path.open("w", newline="", encoding="utf-8") as metrics_handle:
            ...
            prof = None
            prof_enabled = os.environ.get("BPFREE_TORCH_PROFILER", "0") == "1"
            ...
            prof_cm = prof if prof is not None else nullcontext()
            ...
            if prof is not None:
                profiler_dir = output_dir / "torch_profiler"
                profiler_dir.mkdir(parents=True, exist_ok=True)
                trace_path = profiler_dir / f"{phase}.rank{rank}.stage{rank}.trace.json"
                prof.export_chrome_trace(str(trace_path))
                print(
                    f"[TORCH_PROFILER] rank={rank} stage={rank} wrote {trace_path}",
                    flush=True,
                )
    ...
    dist.barrier()
    wall_ms = gpu_transport.sync_max_ms((time.perf_counter() - phase_started) * 1000.0, device)

    optimizer_steps_tensor = torch.tensor(
        [schedule.optimizer_steps],
        dtype=torch.int64,
        device=device,
    )
    dist.all_reduce(optimizer_steps_tensor, op=dist.ReduceOp.MAX)
    logical_optimizer_steps = int(optimizer_steps_tensor.item())
    ...
    if rank == world_size - 1:
        ...
        if perf_minimal_metrics and mode == "train":
            return {
                ...
                "optimizer_steps": logical_optimizer_steps,
                ...
                "window_input_staging": bool(window_input_staging),
            }

        base_summary = pipeline_support.summarize_phase(phase, result_rows, result_path)
        base_summary["window_input_staging"] = bool(window_input_staging)
