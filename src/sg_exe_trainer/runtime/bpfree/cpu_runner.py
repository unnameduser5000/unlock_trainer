#!/usr/bin/env python3
"""Run BPFree over pinned-CPU/Gloo stage transport.

This file is intentionally small:
- no patched old loop
- no compatibility adapter
- no torchrun; this runner still uses mp.spawn internally
- real schedule phase lives in bpfree_pipeline_schedule.phase_v0
"""
...
from sg_exe_trainer.runtime.bpfree.cpu_phase import run_phase_schedule_cpu
...
def evaluator_compatible_trainable_state(
    module: torch.nn.Module,
) -> dict[str, torch.Tensor]:
    """Map stage-local trainable names back to full-model parameter names."""
    ...
    for local_name, param in module.named_parameters():
        ...
        if len(parts) >= 3 and parts[0] == "layers" and parts[1].isdigit():
            ...
            full_name = ".".join(
                ["model", "layers", str(global_layer), *parts[2:]]
            )
        else:
...
def distributed_worker(rank: int, world_size: int, cfg: dict[str, Any]) -> None:
    ...
    configure_link_emulation(
        ...
        seed=int(cfg.get("link_emulation_seed", 0)) + rank,
    )
    ...
    try:
        ...
        if local_hidden_size is None or int(local_hidden_size) <= 0:
            raise ValueError(f"Could not resolve a positive hidden_size for model={resolved_model}")
        local_hidden_size = int(local_hidden_size)
        ...
        if stage0_input_embedding is not None:
            ...
            for p in stage0_input_embedding.parameters():
        ...
        if not bool(cfg["skip_eval_before"]):
            phase_summary = run_phase_schedule_cpu(
                rank=rank,
                world_size=world_size,
                phase="eval_before",
                ...
                input_embedding=stage0_input_embedding,
                ...
                action_trace_start_window=int(cfg["action_trace_start_window"]),
                action_trace_end_window=cfg["action_trace_end_window"],
                perf_minimal_metrics=bool(cfg["perf_minimal_metrics"]),
                recv_prepost_depth=int(cfg.get("recv_prepost_depth", 0)),
                max_pending_send_bytes=int(cfg["max_pending_send_bytes"]),
                max_posted_recv_bytes=int(cfg["max_posted_recv_bytes"]),
        ...
        if bool(cfg["save_trainable_state"]):
            torch.save(
                evaluator_compatible_trainable_state(chunk),
        ...
        if rank == world_size - 1:
            ...
            summary: dict[str, Any] = {
                "runner": "bpfree-cpu-comm-budget-v2",
                "transport": "gloo-cpu-hidden-pinned-budgeted",
                "transport_details": (
                    "BP-free GPU compute + tagged pinned CPU/Gloo forward hidden "
                    "transport with one per-rank pending-send-byte budget and one "
                    "per-rank preposted-receive-byte budget"
                ),
                "recv_prepost_depth": int(cfg.get("recv_prepost_depth", 0)),
                "max_pending_send_bytes": int(cfg["max_pending_send_bytes"]),
                "max_posted_recv_bytes": int(cfg["max_posted_recv_bytes"]),
                "link_emulation": {
                    ...
                    "bandwidth_mbps": float(cfg.get("link_bandwidth_mbps", 0.0)),
                    "jitter_ms": float(cfg.get("link_jitter_ms", 0.0)),
                    "seed": int(cfg.get("link_emulation_seed", 0)),
                },
                ...
                * int(cfg["gradient_accumulation_steps"]),
                ...
                "action_trace_start_window": cfg["action_trace_start_window"],
                "action_trace_end_window": cfg["action_trace_end_window"],
                "perf_minimal_metrics": cfg["perf_minimal_metrics"],
                "trainable_state_saved": cfg["save_trainable_state"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="First logical update window recorded and synchronized.",
    )
    parser.add_argument(
        "--action_trace_end_window",
        type=int,
        default=None,
        help="Exclusive logical update-window limit for action tracing.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum number of tagged receive actions preposted; 0 is blocking.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Save evaluator-compatible per-stage trainable tensors after training.",
    )
...
def main() -> None:
    ...
    hidden_size = (
        ...
        or getattr(model_config, "n_embd", None)
    )
    ...
    cfg = {
        ...
        "save_trainable_state": args.save_trainable_state,
        ...
        "action_trace_start_window": args.action_trace_start_window,
        ...
        "link_emulation_seed": args.link_emulation_seed,
        "vocab_size": vocab_size,
        "hidden_size": hidden_size,
    }

    print(
        "Starting BPFree CPU/Gloo runtime "
        ...
        f"action_trace={args.enable_action_trace} sync_action_trace={args.sync_action_trace} "
        f"trace_windows=[{args.action_trace_start_window},"
        f"{args.action_trace_end_window})",
        flush=True,
    )
