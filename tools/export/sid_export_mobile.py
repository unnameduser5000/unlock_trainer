...
from tools.export.bpfree_pipeline_boundary import (
    audit_joint_graph as audit_pipeline_boundary,
    verify_runtime_operator as verify_pipeline_boundary_operator,
    wrap_with_pipeline_boundary,
)
...
def export_chunk(
    *,
    ...
    export_module: nn.Module = chunk_module
    if enable_pipeline_boundary:
        export_module = wrap_with_pipeline_boundary(chunk_module).train()
        print("      pipeline_boundary=enabled")
    ...
    with sdpa_kernel([SDPBackend.MATH]):
        ep = export(export_module, example_args, strict=False)
        joint_ep = _export_forward_backward(ep)
        boundary_audit = (
            audit_pipeline_boundary(joint_ep) if enable_pipeline_boundary else None
        )
        if dump_joint_graph:
            diagnostic_path = (
                output_dir
    ...
    if enable_pipeline_boundary:
        verify_pipeline_boundary_operator(executorch_program)
    ...
    contract_path = save_path.with_suffix(".runtime_contract.json")
        ...
        json.dumps(
            {
                ...
                "num_chunks": num_chunks,
                "chunk_idx": target_chunk,
                "is_terminal_chunk": chunk_module.is_terminal_chunk,
                "sequence_length": seq_len,
                "batch_size": batch_size,
                "belief_transport_mode": normalized_belief_transport_mode,
                "consumes_previous_log_probs": chunk_module.consumes_prev_log_probs,
                "returns_full_log_probs": chunk_module.returns_full_log_probs,
                "pipeline_boundary": enable_pipeline_boundary,
                "lora_rank": lora_rank,
    ...
    print(f"Runtime contract: {contract_path}")

    if boundary_audit is not None:
        audit_path = save_path.with_suffix(".boundary_audit.json")
        audit_path.write_text(
            json.dumps(boundary_audit, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Boundary audit: {audit_path}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Base export seed. Chunk i uses seed+i so artifacts are reproducible independently.",
    )
    parser.add_argument(
        "--dump_joint_graph",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Insert the BP-free pause marker between local forward outputs and backward. "
            "The resulting PTE requires the custom BPFreeTrainingModule runtime."
        ),
    )
    ...
    for chunk_idx in chunk_indices:
        ...
        export_chunk(
            ...
            export_seed=args.seed + chunk_idx,
            dump_joint_graph=args.dump_joint_graph,
            enable_pipeline_boundary=args.enable_pipeline_boundary,
        )
