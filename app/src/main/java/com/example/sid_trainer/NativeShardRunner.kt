...
data class ShardExecutionResult(
    ...
    val checkpointStep: Long,
    val timing: ShardExecutionTiming,
    val committedReplayShortCircuited: Boolean = false
)

/** Values copied at the BP-free forward boundary and safe to send during local backward. */
data class ShardBoundaryResult(
    val outputHiddenStates: Sid.TensorData,
    val outputShiftLogP: Sid.TensorData,
    val localLoss: Float,
    val evalOnly: Boolean
...
data class ShardBoundaryOutputPolicy(
    val includeHidden: Boolean = true,
    val includeBelief: Boolean = true
)
...
data class ShardExecutionTiming(
    ...
    val forwardBoundaryMs: Long = 0,
    val backwardMs: Long = 0,
    val gradientsMs: Long = 0,
    val optimizerCreateMs: Long = 0,
    val optimizerStepMs: Long = 0,
    val checkpointSaveMs: Long = 0,
    val outputConvertMs: Long = 0,
    val forwardStartedEpochMs: Long = 0,
    val forwardBoundaryEpochMs: Long = 0,
    val backwardStartedEpochMs: Long = 0,
    val backwardCompletedEpochMs: Long = 0
) {
    val totalMeasuredMs: Long
        get() = runtimeAcquireMs + checkpointRestoreMs + inputBuildMs + executeMs +
            gradientsMs + optimizerCreateMs + optimizerStepMs + checkpointSaveMs + outputConvertMs
    ...
    fun describeForLog(): String {
        return "runtimeAcquireMs=$runtimeAcquireMs checkpointRestoreMs=$checkpointRestoreMs " +
            ...
            "forwardBoundaryMs=$forwardBoundaryMs backwardMs=$backwardMs gradientsMs=$gradientsMs " +
            "optimizerCreateMs=$optimizerCreateMs optimizerStepMs=$optimizerStepMs " +
            "checkpointSaveMs=$checkpointSaveMs " +
...
object NativeShardRunner {
    ...
    private const val TRAINING_CHECKPOINT_INTERVAL_STEPS = 1L

    private val cacheLock = ReentrantLock()
    private var cachedModelPath: String? = null
    private var cachedRuntime: LoadedRuntime? = null
    private var scannedModelPath: String? = null
    private var scannedModelHasBoundary = false

    fun execute(
        modelPath: String,
        request: Sid.ForwardChunkRequest,
        boundaryOutputPolicy: ShardBoundaryOutputPolicy = ShardBoundaryOutputPolicy(),
        onBoundary: ((ShardBoundaryResult) -> Unit)? = null
    ): ShardExecutionResult {
        val requestedMode = normalizeRuntimeExecutionMode(request.runtimeExecutionMode)
        val hasBoundary = modelPath.hasPipelineBoundary()
        val effectiveMode = when (requestedMode) {
            "auto" -> if (hasBoundary) "split" else "atomic"
            "split" -> {
                require(hasBoundary) {
                    "Split execution requires a PTE with a BP-free pipeline boundary: $modelPath"
                }
                "split"
            }
            else -> "atomic"
        ...
        Log.i(
            LOG_TAG,
            "Execution mode requested=$requestedMode effective=$effectiveMode boundary=$hasBoundary model=$modelPath"
        )

        if (effectiveMode == "split") {
            releaseStandardRuntime()
            return BPFreeShardRuntime.execute(
                modelPath = modelPath,
                request = request,
                outputPolicy = boundaryOutputPolicy,
                onBoundary = onBoundary
            )
        }
        BPFreeShardRuntime.release()
        ...
        return invocation.toExecutionResult(request, runtimeAcquireMs, effectiveMode)
    }
    ...
    private fun normalizeRuntimeExecutionMode(rawMode: String): String {
        return when (rawMode.trim().lowercase()) {
            ...
            else -> error("Unsupported runtime_execution_mode='$rawMode'; expected auto, atomic, or split.")
        }
    }
    ...
    private fun releaseStandardRuntime() = cacheLock.withLock {
    ...
    internal fun buildBoundaryTrainingInputs(request: Sid.ForwardChunkRequest): Array<EValue> {
        return buildTrainingInputs(request)
    }

    internal fun copyBoundaryTensor(tensor: Tensor): Sid.TensorData {
        return tensor.toSidTensor()
    }

    internal fun boundaryTensorMetadata(tensor: Tensor): Sid.TensorData {
        return Sid.TensorData.newBuilder()
            .setData(ByteString.EMPTY)
            .addAllShape(tensor.shape().map { it.toInt() })
            .setDataType(tensor.dtype().toProtoDataType())
            .build()
    }

    internal fun boundaryScalarValue(tensor: Tensor): Float {
        require(tensor.isScalarFloatingTensor()) {
            "BP-free boundary loss must be a floating-point scalar."
        }
        return tensor.scalarFloatValue()
    }
    ...
    private fun InvocationResult.toExecutionResult(
        ...
        val hiddenStates = if (committedReplayShortCircuited) {
            Sid.TensorData.getDefaultInstance()
        } else {
            remainingTensors.getOrNull(0)?.toSidTensor()
                ?: copyTensorWithData(request.hiddenStates, request.hiddenStates.data.toByteArray())
        }
        val shiftLogP = if (committedReplayShortCircuited) {
            Sid.TensorData.getDefaultInstance()
        } else {
            remainingTensors.getOrNull(1)?.toSidTensor()
        ...
        return ShardExecutionResult(
            ...
            optimizerStepCommitted = optimizerStepCommitted,
            ...
            timing = finalTiming,
            committedReplayShortCircuited = committedReplayShortCircuited
        )
    }
    ...
    private fun String.hasPipelineBoundary(): Boolean {
        return cacheLock.withLock {
            if (scannedModelPath == this) {
                return@withLock scannedModelHasBoundary
            }
            val marker = "sid_bpfree_pipeline".encodeToByteArray()
            val detected = File(this).inputStream().buffered().use { input ->
                val buffer = ByteArray(TRAINING_METADATA_SCAN_BYTES)
                val read = input.read(buffer)
                read > 0 && buffer.indexOf(marker, read) >= 0
            }
            scannedModelPath = this
            scannedModelHasBoundary = detected
            detected
        }
    }
    ...
    private class TrainingLoadedRuntime(
        ...
        private val trainingState = MobileTrainingState(
            modelPath = modelPath,
            parametersProvider = { module.namedParameters(DEFAULT_METHOD) },
            defaultLearningRate = DEFAULT_TRAINING_LEARNING_RATE,
        ...
        override fun execute(request: Sid.ForwardChunkRequest): InvocationResult {
            val prepared = trainingState.prepare(request)
            val effectiveRequest = prepared.request
            if (prepared.shortCircuitLocalExecution) {
                Log.i(
                    LOG_TAG,
                    "Committed request replay short-circuited before forward/backward for $modelPath: " +
                        "request=${prepared.requestKey} step=${trainingState.currentOptimizerStepCount()}"
                )
                return InvocationResult(
                    ...
                    checkpointIntervalSteps = trainingState.checkpointIntervalSteps,
                    optimizerStepCount = trainingState.currentOptimizerStepCount(),
                    checkpointBytes = 0L,
                    checkpointStep = 0L,
                    timing = ShardExecutionTiming(checkpointRestoreMs = prepared.checkpointRestoreMs),
            ...
            val optimizerOutcome = if (prepared.applyOptimizerStep) {
                ...
                trainingState.applyGradients(prepared, gradients)
            } else {
                val reason = if (effectiveRequest.evalOnly) "eval-only request" else "committed request replay"
                Log.i(LOG_TAG, "$reason for $modelPath: skipped namedGradients() and AdamW.step().")
                trainingState.outcomeWithoutOptimizerStep(
            ...
            Log.i(
                ...
                "TrainingModule.executeForwardBackward() succeeded for $modelPath " +
                    ...
                    "optimizerStepApplied=${optimizerOutcome.optimizerStepApplied} gradients=$gradientCount " +
                    "optimizerStepCount=${optimizerOutcome.optimizerStepCount} lr=${prepared.learningRate} " +
                    "checkpointSaved=${optimizerOutcome.checkpointSaved} checkpointStep=${optimizerOutcome.checkpointStep} " +
                    "checkpointBytes=${optimizerOutcome.checkpointBytes} checkpointIntervalSteps=${trainingState.checkpointIntervalSteps} " +
            ...
            for (inputs in candidateInputs) {
                try {
                    ...
                    return InvocationResult(
                        ...
                        learningRate = 0.0,
                        optimizerStepApplied = false,
                        optimizerStepCommitted = false,
                        checkpointSaved = false,
                        checkpointIntervalSteps = TRAINING_CHECKPOINT_INTERVAL_STEPS,
                        optimizerStepCount = 0L,
                        checkpointBytes = 0L,
                        checkpointStep = 0L,
                        timing = ShardExecutionTiming(
                            inputBuildMs = inputBuildMs,
