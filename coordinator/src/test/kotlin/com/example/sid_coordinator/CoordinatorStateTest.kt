...
class CoordinatorStateTest {
    ...
    @Test
    fun durableSubmissionUsesLiveStageZeroWhenDownstreamIsOffline() {
        val state = CoordinatorState(
            initialConfig = testConfig(),
            persistence = CoordinatorPersistence(
                Files.createTempDirectory("sid-coordinator-durable-admission-test")
                    .resolve("coordinator.db")
                    .toString()
            )
        )
        ...
        assertFalse(state.planRequestSubmission().accepted)
        val durablePlan = state.planRequestSubmission(requireCompletePipeline = false)
        assertTrue(durablePlan.accepted)
        assertEquals(0, durablePlan.stageId)
        assertEquals(1, durablePlan.nodeId)
    }
    ...
    @Test
    fun lateBoundaryAckDoesNotRegressCompletedRequestState() {
        val state = CoordinatorState(
            initialConfig = testConfig(),
            persistence = CoordinatorPersistence(
                Files.createTempDirectory("sid-coordinator-completed-state-test")
                    .resolve("coordinator.db")
                    .toString()
            )
        )
        val completed = Sid.RequestEvent.newBuilder()
            .setRequestId("durable-000001")
            .setBatchId(1)
            .setChunkIdx(2)
            .setStageId(2)
            .setNodeId(3)
            .setEventType(Sid.RequestEventType.COMPLETED)
            .setSuccess(true)
            .setTerminal(true)
            .setMessage("Terminal stage completed")
            .build()
        val lateAck = completed.toBuilder()
            .setChunkIdx(1)
            .setStageId(1)
            .setNodeId(2)
            .setEventType(Sid.RequestEventType.BOUNDARY_ACKNOWLEDGED)
            .setMessage("Late upstream boundary ACK")
            .build()

        state.reportRequestEvent(completed)
        state.reportRequestEvent(lateAck)

        val request = state.listRecentRequests(limit = 10, lifecycleState = null).single()
        assertEquals("COMPLETED", request.lifecycleState)
        assertEquals(Sid.RequestEventType.COMPLETED.name, request.latestEventType)
        assertEquals(2, request.latestStageId)
    }
