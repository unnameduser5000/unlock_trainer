...
class CoordinatorRequestOrchestrator(
    ...
    suspend fun submitRequest(
        ...
        if (request.durablePipeline && request.dropOnForwardFailure) {
            return failureResponse(
                request,
                "durable_pipeline and drop_on_forward_failure are mutually exclusive.",
                Sid.RequestFailureKind.REQUEST_FAILURE_KIND_INVALID_REQUEST
            )
        }

        val requestId = request.requestId
        val plan = state.planRequestSubmission(requireCompletePipeline = requiresCompletePipeline(request))
    ...
    private fun failureResponse(
        request: Sid.ForwardChunkRequest,
        message: String,
        failureKind: Sid.RequestFailureKind = Sid.RequestFailureKind.REQUEST_FAILURE_KIND_WORKER_UNAVAILABLE
    ): Sid.ForwardChunkResponse {
        return Sid.ForwardChunkResponse.newBuilder()
            .setSuccess(false)
            .setMessage(message)
