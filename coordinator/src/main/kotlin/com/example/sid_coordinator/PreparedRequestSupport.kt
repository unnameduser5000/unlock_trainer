...
internal fun requiresCompletePipeline(request: Sid.ForwardChunkRequest): Boolean =
    !(request.durablePipeline || request.dropOnForwardFailure)

internal fun isTrainCommitValid(
    evalOnly: Boolean,
    success: Boolean,
    terminal: Boolean,
    processedStageId: Int,
    stageMetrics: List<Sid.StageExecutionMetrics>
): Boolean {
    if (evalOnly) return true
    if (!success || !terminal || processedStageId < 0) return false
    return hasCommittedPrefix(evalOnly, processedStageId, stageMetrics)
}

internal fun isDroppedPrefixCommitValid(
    evalOnly: Boolean,
    success: Boolean,
    terminal: Boolean,
    forwardDropped: Boolean,
    processedStageId: Int,
    firstUnprocessedStageId: Int,
    stageMetrics: List<Sid.StageExecutionMetrics>
): Boolean {
    if (!success || terminal || !forwardDropped || processedStageId < 0) return false
    if (firstUnprocessedStageId != processedStageId + 1) return false
    return hasCommittedPrefix(evalOnly, processedStageId, stageMetrics)
}

private fun hasCommittedPrefix(
    evalOnly: Boolean,
    processedStageId: Int,
    stageMetrics: List<Sid.StageExecutionMetrics>
): Boolean {
    val expectedStageIds = (0..processedStageId).toSet()
    return stageMetrics.map { it.stageId }.toSet() == expectedStageIds &&
        stageMetrics.size == expectedStageIds.size &&
        if (evalOnly) {
            stageMetrics.all { it.evalOnly }
        } else {
            stageMetrics.all { !it.evalOnly && it.optimizerStepCommitted }
        }
}
...
const val DEFAULT_BELIEF_TRANSPORT_MODE = "terminal"
